...
Each of these functions returns the name of a file that does not currently exist. **tmpnam** returns a name unique in the temporary directory and **_tempnam** lets you generate a unique name in a directory other than the temporary one. Note than when a file name is pre-pended with a backslash and no path information, such as \fname21, this indicates that the name is valid for the current working directory.

**_tmpnam** will generate a unique file name for a directory using the rules from [GetTempPathW](https://docs.microsoft.com/en-us/windows/desktop/api/fileapi/nf-fileapi-gettemppathw).
